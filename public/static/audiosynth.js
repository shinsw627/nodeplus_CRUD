...
!(function () {
    ...
    setPub('generate', function (sound, note, octave, duration) {
        ...
        if (
            typeof this._fileCache[sound][octave - 1][note][time] != 'undefined'
        ) {
            if (this._debug) {
...
Synth.loadModulationFunction(
    ...
    function (i, sampleRate, frequency, x) {
        return (
            0.5 * Math.sin(0.5 * Math.PI * ((i / sampleRate) * frequency) + x)
        )
    },
    function (i, sampleRate, frequency, x) {
        return (
            0.5 * Math.sin(0.25 * Math.PI * ((i / sampleRate) * frequency) + x)
        )
    }
)
...
Synth.loadSoundProfile(
    {
        ...
        dampen: function (sampleRate, frequency, volume) {
            return Math.pow(
                0.5 * Math.log((frequency * volume) / sampleRate),
                2
            )
        },
    ...
    },
    {
        ...
        wave: function (i, sampleRate, frequency) {
            ...
            if (valueTable.length <= Math.ceil(period)) {
                ...
                valueTable[playVal] =
                    (valueTable[
                        playVal >= valueTable.length - 1 ? 0 : playVal + 1
                    ] +
                        valueTable[playVal]) *
                    0.5
